...
# Skip if SKIP_REFERENCE=yes (reference benchmarks run separately)
if [[ "${SKIP_REFERENCE:-no}" == "no" ]]; then
	...
	echo ""
	echo "=== Level Zero Native Benchmark ==="
	echo "⏭️  Skipping (SKIP_REFERENCE=yes) - using shared reference results"
fi
