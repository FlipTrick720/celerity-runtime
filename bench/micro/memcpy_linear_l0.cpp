...
#include "common/level_zero_queue.hpp"
#include <vector>
#include <cstring>
#include <ctime>
#include <unistd.h>
...
// Level Zero backend version - replaces SYCL queue with L0 queue
static Row run_one(level_zero_queue& q, const Args& args,
	...
	t0 = clk::now();
	if (args.batch) {
		// Batch mode: submit all copies then wait once
		// Note: Level Zero command lists are executed immediately in this simple wrapper
		// For true batching, we'd need to accumulate commands in a single list
		for (int i=0;i<reps;i++) {
			q.copy(src, dst, bytes);
		}
		q.wait();
	} else {
		// Sync mode: wait after each copy
		for (int i=0;i<reps;i++) {
			q.copy(src, dst, bytes);
			q.wait();
		}
...
int main(int argc, char** argv){
	...
	// Create Level Zero queue instead of SYCL queue
	level_zero_queue q(args.device_index);
	...
	if (args.human) {
		std::cout << "=== memcpy_linear (Level Zero Backend) ===\n";
		std::cout << "Device: " << q.get_device_name() << "\n";
		std::cout << "Backend: level_zero (native)\n";
	...
	void* hsrc = args.pin_host ? q.malloc_host(BIG) : std::aligned_alloc(4096, ((BIG+4095)/4096)*4096);
	void* hdst = args.pin_host ? q.malloc_host(BIG) : std::aligned_alloc(4096, ((BIG+4095)/4096)*4096);
	if(!hsrc||!hdst){ std::cerr<<"host alloc failed\n"; return 1; }
	std::memset(hsrc, 0x5A, BIG);
	std::memset(hdst, 0x00, BIG);
	
	void* dsrc = q.malloc_device(BIG);
	void* ddst = q.malloc_device(BIG);
	if(!dsrc||!ddst){ std::cerr<<"device alloc failed\n"; return 2; }
	
	// warmup
	q.copy(hsrc, dsrc, BIG);
	q.wait();
	q.copy(dsrc, ddst, BIG);
	q.wait();
		...
		for (auto op: { "D2D", "H2D", "D2H" }) {
			...
			if (args.csv) {
				...
				ss<<tstamp<<","<<host_s<<","<<git_sha
					<<",memcpy_linear_l0,level_zero_native,\""<<q.get_device_name()<<"\""
