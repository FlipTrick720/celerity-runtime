...
struct batch_manager {
	...
	void initialize(ze_context_handle_t context, ze_device_handle_t device, ze_command_queue_handle_t q) {
		...
		if (!env_registered) {
			...
			pref.register_variable<size_t>("L0_MICRO_THRESHOLD");
			pref.register_variable<size_t>("L0_SMALL_THRESHOLD");
			pref.register_variable<bool>("L0_USE_BATCHING");
			env_registered = true;
		}
		
		// FIXED: Ensure environment variables are properly consumed
